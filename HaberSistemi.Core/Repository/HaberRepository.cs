...
namespace HaberSistemi.Core.Repository
{
    public class HaberRepository : IHaberRepository
    {
        ...
        {
            return _context.Haber.Select(x => x); //Tüm haberler donecek.
        }

        public Haber GetById(int id)
        {
            return _context.Haber.FirstOrDefault(x => x.ID == id); //Gelen idye gore donecek.
        }

        public IQueryable<Haber> GetMany(Expression<Func<Haber, bool>> expression)
        {
            return _context.Haber.Where(expression); //Birden fazla deger donecek.
        }
