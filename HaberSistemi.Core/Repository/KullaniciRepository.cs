...
namespace HaberSistemi.Core.Repository
{
    public class KullaniciRepository : IKullaniciRepository
    {
        ...
        {
            return _context.Kullanici.Select(x => x); //Tüm kullanıcılar donecek.
        }

        public Kullanici GetById(int id)
        {
            return _context.Kullanici.FirstOrDefault(x => x.ID == id); //Gelen idye gore donecek.
        }

        public IQueryable<Kullanici> GetMany(Expression<Func<Kullanici, bool>> expression)
        {
            return _context.Kullanici.Where(expression); //Birden fazla deger donecek.
        }
